...
Define Would You Rather as a shared-screen social dilemma game with paired-option decks and tap-to-advance play flow.
...
#### Scenario: Choosing an option advances deck

- **WHEN** a user taps either option on the current dilemma after the group has decided
...
- **WHEN** the last dilemma in the deck has been shown and the user taps an option
...
The Would You Rather play phase SHALL display the current dilemma prominently on a single shared screen with both options visible as tappable choices, and session progress.
...
#### Scenario: Options are tappable

- **WHEN** a user is in the Would You Rather play phase viewing a dilemma
- **THEN** each option is a tappable control that advances to the next dilemma when selected
