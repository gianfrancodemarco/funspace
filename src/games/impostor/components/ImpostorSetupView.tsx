...
import {
  getPresetConfig,
  impostorPresets,
  type ImpostorPresetId,
} from "../presets";
import type { ImpostorConfig } from "../types";
import { getDefaultWordPackIds, getWordPacksForLocale } from "../word-packs";

const presetOptions: Array<{
  id: ImpostorPresetId;
  labelKey: "classic" | "withSpy" | "chaos" | "custom";
}> = [
  ...impostorPresets.map((preset) => ({
    id: preset.id,
    labelKey:
      preset.id === "with-spy"
        ? ("withSpy" as const)
...
export function ImpostorSetupView({
  minPlayers,
  maxPlayers,
  onStart,
}: GameSetupProps) {
  const t = useTranslations("impostor.setup");
  const locale = useLocale();
  const { players, isLoading } = usePlayerRoster();
  const wordPacks = getWordPacksForLocale(locale);

  const [selectedNames, setSelectedNames] = useState<string[]>([]);
  const [presetId, setPresetId] = useState<ImpostorPresetId>("classic");
  ...
  useEffect(() => {
    setSelectedPackIds(getDefaultWordPackIds(locale));
  }, [locale]);
    ...
    <div className="space-y-8">
      ...
      {presetId === "custom" && (
        <section className="grid grid-cols-2 gap-4">
          ...
          <label className="space-y-2">
            <span className="text-sm font-medium">{t("spyCount")}</span>
            <input
              type="number"
              min={0}
              max={Math.floor(selectedCount / 3)}
              value={spyCount}
              onChange={(event) =>
                setSpyCount(Number.parseInt(event.target.value, 10) || 0)
              }
              className="border-input bg-background w-full rounded-md border px-3 py-2"
            />
          </label>
