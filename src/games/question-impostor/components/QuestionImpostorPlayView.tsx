...
export function QuestionImpostorPlayView({ session, onComplete }: GamePlayProps) {
  const t = useTranslations("questionImpostor.play");
  const { send } = useGameShell();
  const initialState = getQuestionImpostorState(session.secrets);
  const [state, setState] = useState<QuestionImpostorGameState | null>(
    initialState,
  );
  ...
  function handleConfirmElimination() {
    ...
    send({
      type: "PATCH_SECRETS",
      secrets: { [QUESTION_IMPOSTOR_STATE_KEY]: nextState },
    });
