...
import { validateWouldYouRatherConfig } from "../config.schema";
import {
  getDefaultDilemmaPackIds,
  getDilemmaDecksForLocale,
} from "../dilemma-decks";
import {
  getPresetPackIds,
  wouldYouRatherPresets,
  type WouldYouRatherPresetId,
} from "../presets";
import type { WouldYouRatherConfig } from "../types";

const presetOptions: Array<{
  id: WouldYouRatherPresetId;
  labelKey: "classic" | "partyMix" | "allAges" | "custom";
}> = [
  ...wouldYouRatherPresets.map((preset) => ({
    id: preset.id,
    labelKey: preset.id as "classic" | "partyMix" | "allAges",
  })),
  { id: "custom", labelKey: "custom" },
];

export function WouldYouRatherSetupView({
  minPlayers,
  maxPlayers,
  onStart,
}: GameSetupProps) {
  const t = useTranslations("wouldYouRather.setup");
  const locale = useLocale();
  const { players, isLoading } = usePlayerRoster();
  const dilemmaDecks = getDilemmaDecksForLocale(locale);

  const [selectedNames, setSelectedNames] = useState<string[]>([]);
  const [presetId, setPresetId] = useState<WouldYouRatherPresetId>("partyMix");
  ...
  useEffect(() => {
    setSelectedPackIds(getDefaultDilemmaPackIds(locale));
  ...
  const validationKey = useMemo(() => {
    ...
    return validateWouldYouRatherConfig(
      {
  ...
  function handleStart() {
    ...
    onStart({
      ...
      gameConfig: {
        promptPackIds: selectedPackIds,
        locale,
      } satisfies WouldYouRatherConfig,
    });
  }
